  ...
  async run() {
    ...
    for (let batch = paymentInfoList.splice(0, maxReceivers); batch.length > 0; batch = paymentInfoList.splice(0, maxReceivers)) {
      ...
      if (tokenID == this.Inc.constants.PRVIDSTR) {
        tx = await sender.prv({ transfer: { fee: 100, prvPayments: paymentInfos }}).catch(this.err);
        await sender.waitTx(tx.Response.txId, 3);
        console.log('Sent new transaction', tx);
      } else {
        tx = await sender.token({ transfer: { fee: 100, tokenPayments: paymentInfos, tokenID: tokenID }}).catch(this.err);
        await sender.waitTx(tx.Response.txId, 3);
        console.log('Sent new transaction', tx);
      }
